...
use hippo::{FileStorage, Insight, SearchEngine, HippoStorage};
...
async fn test_storage_integration() {
    ...
    // Test reinforcement
    storage.apply_reinforcement(vec![insight1.uuid], vec![insight2.uuid]).await.unwrap();
...
async fn test_search_parameters() {
    ...
    let insights = vec![
        ...
        Insight::new("Second insight".to_string(), vec!["second".to_string()], 0.7),
...
async fn test_model_loading_performance() {
    ...
    assert!(duration.as_millis() < 2000, "Model loading took too long: {:?}", duration);
}
...
async fn test_embedding_accuracy() {
    ...
    let embedding1 = engine.embed_text("Rust programming language").await.unwrap();
...
async fn test_full_search_integration() {
    ...
    assert!(top_result.relevance > 0.3, "Relevance should be reasonable: {}", top_result.relevance);
