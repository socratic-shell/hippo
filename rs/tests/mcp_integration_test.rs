...
async fn test_mcp_server_startup_and_list_tools() {
    ...
    let transport =
        TokioChildProcess::new(tokio::process::Command::new(&server_path).configure(|cmd| {
            cmd.arg("--memory-dir").arg(memory_dir.as_ref());
        }))
        .expect("Failed to create child process transport");

    // Connect to the server with a timeout
    let client = timeout(Duration::from_secs(10), ().serve(transport))
        .await
        .expect("Server startup timed out")
        .expect("Failed to connect to server");

    // Test 1: List tools - this verifies the server started and can respond
    let tools = timeout(Duration::from_secs(5), client.list_all_tools())
        .await
    ...
    assert!(
        tool_names.contains(&"hippo_record_insight"),
        "Missing hippo_record_insight tool"
    );
    assert!(
        tool_names.contains(&"hippo_search_insights"),
        "Missing hippo_search_insights tool"
    );
    assert!(
        tool_names.contains(&"hippo_modify_insight"),
        "Missing hippo_modify_insight tool"
    );
    assert!(
        tool_names.contains(&"hippo_reinforce_insight"),
        "Missing hippo_reinforce_insight tool"
    );
...
async fn test_mcp_basic_tool_functionality() {
    ...
    assert_eq!(
        record_result.is_error,
        Some(false),
        "Record insight returned error: {:?}",
    ...
    if let Some(content) = record_result.content.first() {
        if let Some(text_content) = content.raw.as_text() {
            assert!(
                text_content.text.contains("Recorded insight with UUID"),
    ...
    assert_eq!(
        ...
        Some(false),
        "Search insights returned error: {:?}",
    ...
    if let Some(content) = search_result.content.first() {
        if let Some(text_content) = content.raw.as_text() {
            ...
            let total_count = search_response["total_count"]
                .as_u64()
            ...
            let insight_content = first_result["insight"]["content"]
                .as_str()
                .expect("Missing content in search result");
            assert_eq!(
                insight_content, "MCP integration test insight",
