...
async def test_relevance_histogram():
    ...
    # Set up in-memory storage for controlled testing
    storage = InMemoryStorage(initial_active_day=1)
    wrapper = TestStorageWrapper(storage)
    
    # Create server with test storage (we'll need to patch this)
    with tempfile.TemporaryDirectory() as temp_dir:
        storage_path = Path(temp_dir) / "hippo.json"
        server = HippoServer(storage_path)
        # Replace with our test storage
...
async def test_temporal_scoring_and_storage():
    ...
    # Set up controlled time environment
    storage = InMemoryStorage(initial_active_day=1)
